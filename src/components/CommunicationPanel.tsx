...
const CommunicationPanel = ({ shop }: CommunicationPanelProps) => {
      ...
      setMessages([
        {
          ...
          sender: 'seller',
          text: isRequestOrder
            ? `Hello! Thanks for contacting ${shop.name}. Please share your required quantity and timeline. We will confirm delivery date after reviewing your order request.`
            : `Hello! Thanks for contacting ${shop.name}. Share your quantity, destination, and timeline so I can send a quote.`,
          time: new Date()
        }
      ]);
    }, 1000);
  ...
  const handleSendMessage = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      const replies = [
        "Great request. What quantity do you need?",
        "I can prepare a quote with lead time and delivery options.",
        "Would you like pricing for wholesale or retail quantity?",
    ...
    return (
      <div className="communication-panel">
        <div className="offline-notice">
          ...
          <h3>Seller Unavailable</h3>
          <p>This seller is currently offline. Please try again later or contact another supplier.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="communication-panel">
        <div className="panel-header">
          ...
          <p>Start a negotiation with {shop.name}</p>
        </div>
        ...
        <div className="communication-options">
          <button onClick={handleStartChat} className="comm-option-btn chat">
            ...
            <span>{isRequestOrder ? 'Start Order Request Chat' : 'Start Quote Chat'}</span>
          </button>
        ...
        <div className="features-list">
          <div className="feature-item">
            ...
            <span>Real-time quote messaging</span>
          </div>
          <div className="feature-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#28a745" strokeWidth="2">
              <polyline points="20 6 9 17 4 12" />
            </svg>
            <span>Invoice and document sharing</span>
          </div>
          <div className="feature-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#28a745" strokeWidth="2">
              <polyline points="20 6 9 17 4 12" />
            </svg>
            <span>{isRequestOrder ? 'Seller confirms delivery schedule after request approval' : 'Delivery and pickup coordination'}</span>
          </div>
        </div>
      </div>
    );
          ...
          <div className="chat-actions">
            ...
            <form onSubmit={handleSendMessage} className="message-form">
              <input
                ...
                placeholder="Ask about MOQ, delivery terms, or pricing..."
          ...
          <div className="call-display">
            {activeCommunication.type === 'video' ? (
              ...
              <div className="voice-call">
                ...
                <h3>{shop.name}</h3>
                <p className="call-status">Voice negotiation in progress</p>
              </div>
            )}
          </div>
