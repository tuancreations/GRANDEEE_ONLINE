      ...
      <div className="shop-detail-container">
        ...
        <section className="storefront-section">
          <div className="section-header-with-action">
            <h2>Storefront Links & Buyer Channels</h2>
            <span>Visible to buyers</span>
          </div>

          <div className="public-links-grid">
            <button type="button" className="public-link-card website-card" onClick={() => handleOpenLink(publicProfile.website, 'website')}>
            ...
            {publicProfile.socialLinks.map((link) => (
              <button
                key={link.label}
                type="button"
                className="public-link-card social-card"
                onClick={() => handleOpenLink(link.url, 'social')}
              >
          ...
          <div className="channel-follow-grid">
            {publicProfile.channels.map((channel) => {
              const following = isChannelFollowed(shop.id, channel.id);

              return (
                <article key={channel.id} className="channel-follow-card">
                  <div className="channel-follow-top">
                    <div>
                      <h3>{channel.name}</h3>
                      <p>{channel.handle}</p>
                    </div>
                  ...
                  <div className="channel-follow-footer">
                    <span>Updated {channel.lastUpdate}</span>
                    <button
                      type="button"
                      className={`follow-channel-btn ${following ? 'following' : ''}`}
                      onClick={() => toggleChannelFollow(shop.id, channel.id)}
                    >
