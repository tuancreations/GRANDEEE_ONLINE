    ...
    <div className="all-shops-page">
      <div className="all-shops-container">
        <div className="page-header">
          <h1>Verified Seller Directory</h1>
          <p>Compare trusted marketplace sellers before you request pricing or place an order.</p>
        </div>
              ...
              <Link to={`/shop/${shop.id}`} key={shop.id} className="shop-card">
                ...
                <div className="shop-card-body">
                  ...
                  <div className="shop-card-links">
                    <button
                      type="button"
                      className="shop-link-chip website-chip"
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        openLink(shop.id, publicProfile.website, 'website');
                      }}
                    >
                      Website
                    </button>

                    {publicProfile.socialLinks.slice(0, 3).map((link) => (
                      <button
                        key={link.label}
                        type="button"
                        className="shop-link-chip social-chip"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          openLink(shop.id, link.url, 'social');
                        }}
                      >
                        {link.label}
                      </button>
                  ...
                  <p className="shop-card-channel-count">
                    {publicProfile.channels.length} buyer update channels
                  </p>
