...
type PublicStorefrontFormState = {
  website: string;
  instagram: string;
  facebook: string;
  x: string;
...
const initialPublicStorefrontFormState: PublicStorefrontFormState = {
  website: '',
  instagram: '',
  facebook: '',
  x: '',
...
const initialChannelFormState: ChannelFormState = {
  name: '',
  handle: '',
  description: ''
};
...
const SellerDashboard = () => {
  ...
  useEffect(() => {
    const nextShopId = managedShops.some((shop) => shop.id === managedShopId)
      ? managedShopId
      : managedShops[0]?.id ?? mockShops[0].id;
    setManagedShopId(nextShopId);
  ...
  useEffect(() => {
    ...
    const getHandle = (label: string) => publicProfile.socialLinks.find((link) => link.label === label)?.handle ?? '';

    setPublicStorefrontForm({
      website: publicProfile.website,
      instagram: getHandle('Instagram'),
      facebook: getHandle('Facebook'),
      x: getHandle('X'),
      tiktok: getHandle('TikTok'),
  ...
  const buildSocialLink = (label: string, handle: string) => {
    ...
    if (label === 'Instagram') {
      ...
      url = `https://facebook.com/${normalized}`;
    } else if (label === 'X') {
      url = `https://x.com/${normalized}`;
    } else if (label === 'TikTok') {
      url = `https://tiktok.com/@${normalized}`;
  ...
  const handleSaveStorefrontProfile = () => {
    updateShopPublicProfile(managedShop.id, {
      website: publicStorefrontForm.website.trim() || publicProfile?.website || '',
      socialLinks: [
        buildSocialLink('Instagram', publicStorefrontForm.instagram),
        buildSocialLink('Facebook', publicStorefrontForm.facebook),
        buildSocialLink('X', publicStorefrontForm.x),
        buildSocialLink('TikTok', publicStorefrontForm.tiktok),
        buildSocialLink('YouTube', publicStorefrontForm.youtube)
      ].filter((item): item is NonNullable<typeof item> => Boolean(item))
    });
    alert('Storefront links updated. Buyers will see the changes on the seller profile pages.');
  };

  const handleAddChannel = () => {
    if (!channelForm.name.trim() || !channelForm.handle.trim() || !channelForm.description.trim()) {
      alert('Please fill in the channel name, handle, and description.');
      return;
    }

    addShopChannel(managedShop.id, {
      name: channelForm.name.trim(),
      handle: channelForm.handle.trim(),
      description: channelForm.description.trim()
    });

    setChannelForm(initialChannelFormState);
  ...
  const analyticsCards = [
    ...
    { title: 'Social Clicks', value: publicProfile?.analytics.socialClicks ?? 0, hint: 'Clicks on social handles' },
    { title: 'Buyer Inquiries', value: publicProfile?.analytics.inquiries ?? 0, hint: 'Requests and messages' },
    { title: 'Followers', value: publicProfile?.analytics.followers ?? 0, hint: 'Public channel followers' },
    { title: 'Channel Subscribers', value: publicProfile?.analytics.channelSubscribers ?? 0, hint: 'Subscribers across updates' }
  ];
      ...
      <div className="seller-dashboard-container">
        <section className="seller-hero">
          ...
          <div className="seller-hero-highlight">
            ...
            <p className="seller-highlight-small">Managing: {managedShop.name}</p>
          </div>
        </section>
        ...
        <section className="seller-public-grid">
          <article className="seller-card storefront-card">
            ...
            <p className="storefront-note">
              Keep your website, social handles, and public shop updates current so buyers can discover and follow your store.
            </p>

            <div className="seller-form-row">
              <div>
                <label>Public Shop</label>
              ...
              <div>
                <label>Website</label>
                <input
                  value={publicStorefrontForm.website}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, website: e.target.value }))}
            ...
            <div className="seller-form-row">
              <div>
                ...
                <input
                  value={publicStorefrontForm.instagram}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, instagram: e.target.value }))}
              ...
              <div>
                ...
                <input
                  value={publicStorefrontForm.facebook}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, facebook: e.target.value }))}
                  placeholder="yourstore"
                />
              </div>
            </div>
            ...
            <div className="seller-form-row">
              <div>
                ...
                <input
                  value={publicStorefrontForm.x}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, x: e.target.value }))}
                  placeholder="@yourstore"
                />
              </div>
              ...
              <div>
                ...
                <input
                  value={publicStorefrontForm.tiktok}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, tiktok: e.target.value }))}
                  placeholder="@yourstore"
                />
              </div>
            </div>
            ...
            <div className="seller-form-row">
              <div>
                ...
                <input
                  value={publicStorefrontForm.youtube}
                  onChange={(e) => setPublicStorefrontForm((prev) => ({ ...prev, youtube: e.target.value }))}
                  placeholder="@yourstore"
                />
              </div>
            ...
            <button type="button" className="seller-submit-btn" onClick={handleSaveStorefrontProfile}>
              Save Website & Social Links
            </button>

            <div className="storefront-links-preview">
              {publicProfile?.socialLinks.map((link) => (
                <span key={`${managedShop.id}-${link.label}`} className="listing-tag">
          ...
          <article className="seller-card storefront-card analytics-card">
            <div className="section-heading-row">
            ...
            <p className="storefront-note">
              Track how buyers interact with your storefront, links, and channels.
            </p>

            <div className="analytics-grid">
              {analyticsCards.map((card) => (
                <article key={card.title} className="analytics-tile">
        ...
        <section className="seller-card channels-card">
          <div className="section-heading-row">
            <h2>Buyer Update Channels</h2>
            <span>Followers can opt in</span>
          </div>

          <p className="storefront-note">
            Create channels for new product drops, price alerts, restocks, or seasonal promotions so buyers can follow your updates.
          </p>
          ...
          <textarea
            ...
            placeholder="Tell buyers what they will receive when they follow this channel"
          />

          <button type="button" className="seller-submit-btn" onClick={handleAddChannel}>
            Create Buyer Channel
          </button>

          <div className="channels-grid">
            {publicProfile?.channels.map((channel) => (
              <article key={channel.id} className="channel-card">
                <div className="channel-card-top">
                  ...
                  <span className={`channel-status ${channel.active ? 'active' : 'inactive'}`}>
                ...
                <div className="channel-card-meta">
                  ...
                  <span>Updated {channel.lastUpdate}</span>
                </div>
              </article>
            ))}
          </div>
